...
def iquest_collection_metadata(
    collection, metaname=None, metavalue=None, metaunit=None, recursive=False
):
    ...
    iquest = IrodsCommand(
        "iquest",
        ["--no-page", "%s'%s'%s'%s"],
        output_filter=iquest_filter2, verbose=False
    )
    ...
    select_cmd = (
        select + " where " + " and ".join(conditions + [collection_condition])
    )
    ...
    if recursive:
        ...
        select_cmd = (
            select
            + " where "
            + " and ".join(conditions + [recursive_condition])
        )

...
class UnitConverter:
    ...
    def __call__(self, value):
        ...
        fmt = "{0:1f}{1}"
        if "{0:.1f}".format(value).endswith(".0"):
            fmt = "{0:.0f}{1}"
...
def options_parser_add_size_units(parser, dest="converter"):
    ...
    parser.add_option("-k", "--kilobytes", const=K,
                      action="store_const", dest=dest,
                      help="print sizes in kilobytes")
    parser.add_option("-m", "--megabytes", const=M,
                      action="store_const", dest=dest,
                      help="print sizes in megabytes")
    parser.add_option("-g", "--gigabytes", const=G,
                      action="store_const", dest=dest,
                      help="print sizes in gigabytes")
    parser.add_option("-t", "--terabytes", const=T,
                      action="store_const", dest=dest,
